...
ENDPOINT_NAME = os.environ["xgboost-bikerental-v1"]
...
def lambda_handler(event, context):
    try:
        ...
        transformed_data = [
            transform_data(instance["features"]) for instance in request["instances"]
        ]
        ...
        print(result)
        result = result.split("\n")
        predictions = [math.expm1(float(r)) for r in result[:-1:]]
